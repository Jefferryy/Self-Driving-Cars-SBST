...
class Response(Enum):
    def __add__(self, b):
        return Response(min(3, self.value + b.value))
...
class Dangerous(Enum):
    def __add__(self, b):
        return Dangerous(min(3, self.value + b.value))
    Safe = 0
...
def find_blame_times(traj1, traj2, lat_params, long_params):
    ...
    for i in range(len(traj1)):
        ...
        t = c1.t
        if is_long_dangerous(c1, c2, long_params) and is_lat_dangerous(c1, c2, lat_params):
